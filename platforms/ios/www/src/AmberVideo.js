define("amber-ambercordova/AmberVideo", ["amber/boot", "amber_core/Kernel-Objects"], function($boot){"use strict";
var $core=$boot.api,nil=$boot.nil,$recv=$boot.asReceiver,$globals=$boot.globals;
$core.addPackage('AmberVideo');
$core.packages["AmberVideo"].innerEval = function (expr) { return eval(expr); };
$core.packages["AmberVideo"].transport = {"type":"amd","amdNamespace":"amber-ambercordova"};

$core.addClass('AmberVideo', $globals.Object, ['cssId', 'videoElement', 'playAudio', 'video'], 'AmberVideo');
$core.addMethod(
$core.method({
selector: "autobuffer:",
protocol: 'starting',
...
//>>excludeEnd("ctx");
$1=$recv(window)._navigator();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx2.sendIdx["navigator"]=1;
//>>excludeEnd("ctx");
return $recv($1)._at_ifPresent_(each,(function(block){
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
return $core.withContext(function($ctx3) {
//>>excludeEnd("ctx");
return $recv($recv(window)._navigator())._at_put_("getUserMedia",block);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
...
$2=$recv(window)._at_put_("URL",block);
throw $early=[$2];
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
...
//>>excludeStart("ide", pragmas.excludeIdeData);
args: [],
source: "configureForBrowser\x0a\x0a\x09\x22specify the browser specific functionalities for further usage\x22\x0a\x0a\x09#( 'getUserMedia' 'webkitGetUserMedia' 'mozGetUserMedia' 'msGetUserMedia')\x0a\x09\x09do:[:each |\x0a\x09\x09\x09window navigator at: each ifPresent:[:block | window navigator at: 'getUserMedia' put: block ].\x0a  \x09\x09].\x0a\x0a   #( 'URL' 'webkitURL' 'mozURL' 'msURL')\x0a\x09\x09do:[:each |\x0a\x09\x09\x09window at: each ifPresent:[ :block | block basicAt: 'klass' put: nil asJSON.\x0a\x09\x09\x09\x09\x09\x09\x09\x09\x09\x09\x09\x09\x09\x09\x09\x09\x09^window at: 'URL' put: block ].\x0a  \x09\x09].",
referencedClasses: [],
//>>excludeEnd("ide");
messageSends: ["do:", "at:ifPresent:", "navigator", "at:put:", "basicAt:put:", "asJSON"]
}),
$globals.AmberVideo);
...
$1=$recv(window)._navigator();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["navigator"]=1;
//>>excludeEnd("ctx");
$recv($1)._at_ifAbsent_("getUserMedia",(function(){
throw $early=[self];

}));
$recv($recv(window)._navigator())._getUserMedia_success_error_($globals.HashedCollection._newFromPairs_(["video",true,"audio",self._playAudio()]),self._videoProcess(),self._errorCallback());
return self;
}
catch(e) {if(e===$early)return e[0]; throw e}
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
...
//>>excludeStart("ide", pragmas.excludeIdeData);
args: [],
source: "configureVideo\x0a\x0a   window navigator at: 'getUserMedia' ifAbsent:[^self]. \x22you must specify the right method for your browser\x22\x0a\x0a   window navigator getUserMedia: #{'video' -> true.  'audio' -> self playAudio} success: self videoProcess error: self errorCallback",
referencedClasses: [],
//>>excludeEnd("ide");
messageSends: ["at:ifAbsent:", "navigator", "getUserMedia:success:error:", "playAudio", "videoProcess", "errorCallback"]
}),
$globals.AmberVideo);
...
//>>excludeStart("ide", pragmas.excludeIdeData);
args: ["html"],
source: "renderOn: html\x0a\x0a\x09self cssId: 'videoOutput'.\x0a\x0a\x09html  video class: 'view';\x0a\x09\x09\x09\x09\x09\x09style: 'display:block';\x0a\x09\x09\x09\x09\x09\x09id: self cssId.\x0a\x0a\x09self configureVideo",
referencedClasses: [],
//>>excludeEnd("ide");
...
//>>excludeStart("ide", pragmas.excludeIdeData);
args: ["aStreamOrURL"],
source: "src: aStreamOrURL\x0a\x0a\x09self videoElement at: 'mozSrcObject' ifPresent:[ ^self videoElement mozSrcObject: aStreamOrURL ].\x0a\x0a\x09(PlatformInterface existsGlobal: 'URL' ) ifFalse: [^self].\x0a\x0a  \x09(PlatformInterface globals URL createObjectURL: aStreamOrURL)\x0a            ifNil: [ self videoElement src: aStreamOrURL ]\x0a            ifNotNil: [ :streamObj | self videoElement src: streamObj ]",
